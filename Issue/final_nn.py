...
def lrcalculation(course):
	...
	numpy.random.seed(7)
	# load the dataset
	dataframe = read_csv("HR_Data.csv", usecols=[4], engine='python')
	...
	model = load_model(course+'_RNN.h5')
	...
	lines = list(r)
	lines[no_rows-1][3] = testPredict[len(testPredict)-1][0]
	f.close()
	...
	model1 = load_model(course+'_ANN.h5')
