...
df = read_csv('MM_Data.csv', usecols=[0,1,2,4,5], engine='python') 
...
while(iterno < 100):
	...
	df = dataframe[0:iterno-1]
	target = target1[0:iterno-1]
	print(df)
	print(target)
	...
	pred = numpy.reshape(dataframe[iterno-1], (dataframe[iterno-1].shape[0], 1))
	predictions = lm.predict(pred)

	target1[iterno-1] = predictions[0][0]
